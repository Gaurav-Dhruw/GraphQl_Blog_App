            ...
            <div className='shadow-lg pb-12 md:pb-10 h-100 rounded-lg bg-white mx-auto w-[18rem] md:w-60 my-10 mb-16' >
                ...
                <div className='p-4 '>
                        ...
                        {post.categories.map(category => (
                            <div key={category.slug} className='mr-3 my-2'>
                                <Link href={`/category/${category.slug}`}>
                                    <span className='transform inline-block duration-200 px-3 py-1  bg-slate-200 rounded-full hover:bg-slate-300'>
                                        {category.name}
                                    </span>
                                </Link>
                            </div>
                    ...
                    <Link href={`/post/${post.slug}`}>
                        <h3 className='mb-3 font-bold max-h-12 overflow-hidden text-ellipsis hover:text-pink-500'>
                            {post.title}
                        </h3>
                    </Link>
